...
### `ImgBuilderService`: `autoFit` su `buildCanvasWithCaption` — mai più ellissi sul testo generato

Portato dal Dominio di un figlio (Br1Gaming), che aveva risolto lo stesso problema con un servizio di progetto a parte (`ShareCardService`, wrapper su `buildCanvasWithCaption`): senza sapere a priori quanto testo arriverà (es. un generatore), l'ellissi di troncamento del comportamento di base è inaccettabile — meglio far crescere il canvas. Quel wrapper duplicava però un dettaglio interno dell'Engine copiato a mano (il tetto "60% di canvasH" di `buildCaption`, come costante isolata invece che letta dalla fonte) e ri-misurava il testo su un canvas temporaneo indipendente, da cui un margine di sicurezza (8%) per assorbire l'eventuale scarto tra le due misure.

- Nuova opzione `autoFit` (booleana) su `CaptionOverlayOptions`: se true, `buildCanvasWithCaption` calcola da sé l'altezza del canvas e i `maxLines` perché `text`/`subtitle` entrino SEMPRE per intero — `imgOpts.height`, se passato, viene ignorato. Nuova `minImageRatio` (default 0.15) per lasciare comunque visibile una quota minima di immagine sopra la fascia di testo su testi molto lunghi.
- Nuovo `ImgBuilderService.fitCaptionHeight` (statico, puro, SSR-safe): stessa matematica di `buildCaption` (stesso `PILL_SUBTITLE_GAP_RATIO`, nuova costante condivisa `CAPTION_MAX_TEXT_HEIGHT_RATIO` al posto del letterale `0.6` prima duplicabile solo a mano) — nessun margine di sicurezza: chi chiama `autoFit` misura con un `ctx` temporaneo ma con lo stesso font che disegnerà poi il risultato finale, e `ctx.measureText` non dipende dalle dimensioni del canvas, quindi la misura è già esatta.
- Non breaking, additivo — comportamento di base (`autoFit` assente/false) invariato.
- Verificato: `tsc --noEmit` pulito; verifica isolata della sola logica pura (`wrapText`/`fitCaptionHeight`/`buildCaption`, nessun DOM) su tre lunghezze di testo crescenti — nessuna ellissi, `canvasH` cresce monotono.
