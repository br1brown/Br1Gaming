...
> I flag di comportamento (`showNav`, `showFooter`, `showPanel`, `fixedTopHeader`, `showNotifications`, `panelForcedLight`, `isWebApp`, `onlyPlainImage`, il `showInHeader` di `loginPage`) sono struttura e vivono in `frontend/src/app/site.ts` (`shell` / `isWebApp` / `onlyPlainImage` / `loginPage`). L'icona di brand nella navbar non è più un flag statico qui, ma `ShellNavResolver.brandIcon` in `nav.ts` (dato risolto a runtime, vedi frontend/README.md).
