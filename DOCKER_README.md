...
| `site.colorTema` | — | Colore tema (hex): palette OKLCH + tono testo — l'UNICO colore che vive ancora qui, tutto il resto della palette (override, `customPalette`) è del design system attivo |
| `Custom` | `{}` | Valori liberi leggibili da backend (`IConfiguration["Custom:..."]`) e Node SSR (`getBr1Settings().Custom`) |

> I flag di struttura/aspetto (`showNav`, `showFooter`, `showPanel`, `fixedTopHeader`, `panelSurface`, `ogImagePlain`, `showBreadcrumb`, `pageFade`, `showBrandIcon`, l'effetto `smoke`, gli override colore `colorSecondary`/`colorBackground`/`colorText`/`colorInfo`/`customPalette`) non vivono più in `global-settings.json` né in `site.ts` a fianco: sono TUTTI campi del design system attivo (`DesignSystemPreset`, `frontend/src/app/core/engine/design-system-presets.ts`), scelto o esteso in `frontend/src/app/site.ts` (`shell.designSystem`) — vedi frontend/README.md §"Preset di Design System"/§"Ruoli di Pagina". Restano invece flag veri e propri di `site.ts`: `shell.showNotifications`, `isWebApp` e il `showInHeader` di `loginPage` (nessuno dei tre è estetico). L'icona di brand nella navbar è `ShellNavResolver.brandIcon` in `nav.ts`, dato risolto a runtime (SE mostrarla resta invece `DesignSystemPreset.showBrandIcon`).
...
| `FONTS_DIR` | `/app/fonts` | Cartella in cui il server cerca il file dichiarato in `DesignSystemPreset.customFont` (design system attivo) — vedi sotto. Il default coincide già col mount point Docker; da impostare solo per uno sviluppo locale con un percorso diverso |
...
Metti il file font in `./fonts` (host, accanto a `global-settings.json`), poi dichiaralo come `defaultFont` nel patch di `extendDesignSystem` del design system attivo — un `CustomFontDef` pieno, scritto lì direttamente (`{ key, family, faces }`, `faces` almeno la regular): `defaultFont: { key: 'brand', family: 'MiaFont', faces: [{ file: 'MiaFont.woff2', weight: 400, style: 'normal' }] }`. `docker-compose.yml` monta quella cartella in sola lettura su `/app/fonts`. Nessun font caricato in `defaultFont`, o il file assente dalla cartella: nessun cambiamento, resta lo stack di font di sistema di oggi (un `SystemFont` del catalogo, già installato nel container — 11 voci — funziona altrettanto come `defaultFont`, senza alcun file da montare). Il font sostituisce sia quello del sito sia quello delle immagini Open Graph (`/cdn-cgi/preview`) — non solo il web: per le immagini, il server chiede a `fc-scan` il nome REALE che fontconfig assegna al file (non necessariamente uguale a `family`, che è solo l'etichetta `@font-face` per il browser) — senza questa corrispondenza Sharp/librsvg ripiegherebbero in silenzio sul font di sistema pur avendo il file corretto sotto mano. Per un percorso host diverso da `./fonts`: `export BR1_FONTS_DIR=/percorso/font; ./scripts/deploy.sh`. Dettagli (`addonFonts`, `ogTextTransform` per personalizzare la sola immagine OG) in [frontend/README.md](frontend/README.md) §"Font: `SystemFont` + `addonFonts`".
