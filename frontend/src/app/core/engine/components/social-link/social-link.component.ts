...
const SOCIAL_DISPLAY_NAMES: Record<string, string> = {
    ...
    twitter: 'X', // rebranding: la chiave interna resta 'twitter' (icona/urlBase invariati), l'etichetta segue il nome attuale
};
...
/** Icona brand per una rete riconosciuta (chiave = `type`/`socialKey`, es. "github", "spotify");
 *  `undefined` se la rete non è nota — usata anche fuori da questo componente per risolvere
 *  l'icona di un `ProjectLink` (vedi `ctaGlyph`/`linkIcon` in cv.component.ts/social.component.ts),
 *  senza duplicare SOCIAL_MAP altrove. */
