...
- **Card testuale** — quando non c'è un'immagine di sfondo: favicon in alto a sinistra (piccolo marchio d'identità, SENZA nome app accanto — i crawler social lo mostrano già nel proprio chrome UI, ripeterlo dentro l'immagine è ridondante) + titolo grande allineato a sinistra + subline opzionale (una riga, troncata con ellissi). Sfondo sul colore brand nudo (`colorTema`, invariato da customPalette/vividezza — quegli override toccano solo superfici derivate, mai il colore sorgente): il testo overlay riceve nero o bianco, qualunque dei due dia il contrasto migliore (`ImgBuilderService.getReadableTextColor`), come ogni altro testo su sfondo colorato del sito — nessuna via "neutra" separata, nessuno scurimento aggiuntivo rispetto al colore scritto in `global-settings.json`.
