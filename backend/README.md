...
- **Nessun throttling né deduplica**, né qui né in `ClientErrorReportingService` lato browser: ogni occorrenza — anche la stessa eccezione ripetuta molte volte in rapida successione (uno script di terze parti in loop, un `effect()` che rientra) — genera una `POST` verso il webhook a parte. Entrambi i punti in cui l'Engine accoda la segnalazione (`ApiExceptionHandler`, `EngineClientErrorController`) chiamano `BackgroundQueue.TryEnqueue` senza controllarne l'esito: se la coda condivisa è già satura (oltre 1000 task pendenti, §7) la segnalazione di errore viene scartata in silenzio invece di quella singola richiesta fallire in modo osservabile.
...
**Tetto di decodifica indipendente dal limite di upload**: `?webopt=true` rifiuta con 422 un'immagine oltre 40 megapixel totali (guardia "decompression bomb" prima di allocare il raster — vedi `MaxDecodePixels` in `EngineBlobController.cs`). Un file può restare comodamente sotto `MaxUploadSizeBytes` (pochi MB, un PNG molto comprimibile o uno scan) e superare comunque questo tetto in risoluzione: l'upload riesce, ma ogni richiesta successiva con `?webopt=true` su quello slug fallisce — l'unica via resta servire l'originale (senza `webopt`), che il chiamante deve gestire come fallback esplicito, l'Engine non lo fa da solo.
